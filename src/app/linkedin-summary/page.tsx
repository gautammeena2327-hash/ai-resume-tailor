...
import { Copy, Download, Loader2, Crown, FileText, ArrowLeft } from 'lucide-react'
import Link from 'next/link'

const FREE_SUMMARIES_LIMIT = 2
...
export default function LinkedInSummaryPage() {
  ...
  const [summaryCount, setSummaryCount] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('linkedinSummaryCount')
      const lastReset = localStorage.getItem('linkedinLastReset')
      const now = new Date().getTime()
      
      if (lastReset && now - parseInt(lastReset) > 30 * 24 * 60 * 60 * 1000) {
        localStorage.setItem('linkedinSummaryCount', '0')
        localStorage.setItem('linkedinLastReset', now.toString())
        return 0
      }
  ...
  const handleSubmit = async () => {
    if (!resume || summaryCount >= FREE_SUMMARIES_LIMIT) return

    setIsLoading(true)
    setError('')
    try {
      const response = await fetch('/api/generate-linkedin-summary', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(data.error || data.details || 'Failed to generate LinkedIn summary')
      }
      setLinkedInSummary(data.linkedInSummary || '')
      setIsFallback(data.isFallback || false)
      const newCount = summaryCount + 1
      setSummaryCount(newCount)
      localStorage.setItem('linkedinSummaryCount', newCount.toString())
    } catch (error: unknown) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-purple-900 dark:to-indigo-900 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300">
              Generate a professional LinkedIn About section from your resume
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-center items-center gap-6 text-sm mb-8">
          <div className="px-4 py-2 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-lg shadow">
            <span className="text-gray-500 dark:text-gray-400">{summaryCount}/{FREE_SUMMARIES_LIMIT} free summaries used</span>
          </div>
        ...
        {!showTool && (
          <div className="text-center">
            ...
            >
              ...
              Create My LinkedIn Summary
            </button>
            <p className="text-gray-500 dark:text-gray-400 mt-4">
              AI-powered LinkedIn About section generation
            </p>
          </div>
        )}
          ...
          <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-3xl shadow-2xl p-6">
            ...
            <div className="mt-6 text-center">
              ...
              <button
                ...
                disabled={!resume || isLoading || summaryCount >= FREE_SUMMARIES_LIMIT}
              ...
              >
                ...
                ) : summaryCount >= FREE_SUMMARIES_LIMIT ? (
                  'Upgrade to continue'
                ) : (
          ...
          <div className="mt-8 bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-3xl shadow-2xl p-6">
            {isFallback && (
              <div className="mb-4 p-3 bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 rounded-lg text-sm">
                Note: OpenAI API unavailable. Showing template summary.
              </div>
            )}
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 flex items-center shadow-lg"
                >
